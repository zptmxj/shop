...
function Racing()
{
            ...
            .then((betAll)=>{
                ...
                .then((gamAll)=>{
                    let data = betAll.map((b)=>{
                        return gamAll.filter(g=>g.Racingidx==b.idx);
                    });
                    console.log(data);
                    setGambler(gamAll);
    ...
    const onEndOk = ()=>{
        ...
        else if(!ready)
        {
            ...
            let end = {Racingidx:modaldata.idx ,option:btnSel, rate:rate};
            console.log("/end_Racing",end);

            fetch(serverPath()+"/end_Racing",{
                method:"post",
                headers : {
                    "content-type" : "application/json",
                },
    ...
    const onInputOk = ()=>{
        ...
        else
        {
            ...
            .then((pjson)=>{
                try {
                    let gamblerjson = {Racingidx:modaldata.idx,uid:userId,name:userName,opnumber:btnSel,point:inputValue};
    ...
    const onAddOk = ()=>{
        ...
        else
        {
            ...
            let RacingData = {title:titleText, uid:userId, options:options, optext:opText}

            fetch(serverPath()+"/in_Racing",{
                method:"post",
                headers : {
                    "content-type" : "application/json",
                },
                body : JSON.stringify(RacingData),
            })
            .then((res)=>res.json(res))
            .then((json)=>{
                console.log(json);
    ...
    return(
        <div className="Racing-w">
            ...
            <>
                {Racings.map((e,i)=>{
                    return (<RacingList key={i} info={e} sel={i} onInputShow={onInputShow} onEndShow={onEndShow} proceeding={proceeding}/>)
                })}
            </>
            ...
            >
                ...
                <Modal.Body >
                    ...
                    <div className='memlist-input mb-3'>
                        <p className="mb-0 mr-1">{"Racing point : "}</p>
...
function RacingList(props)
{
    ...
    if(info.hasOwnProperty("points"))
    {
        ...
        {
            //console.log("RacingList",i,info.option);
...
function PopList(props) {
    ...
    let rt = list.map((e,j)=>{
        ...
        if(e.opnumber == i)
        return(
        <Form.Group as={Row} key = {j} controlId="formUser" className="Racing-poplist m-1">
            <Col xs={7} className='Attend-text-right p-0'>
                <NamePlate mem={mem[0]} />
            </Col>
            <Col xs={4} className='Racing-list p-0'>
